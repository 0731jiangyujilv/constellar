...
import { isSupportedChain, SUPPORTED_CHAIN_LIST } from '@/config/chains'
  ...
  if (isConnected && address) {
    return (
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-3">
          ...
          {/* {!isAuthenticated && !isWrongNetwork && (
            <button
              ...
              className="rounded-full bg-[var(--color-cyan)] px-4 py-2 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-50"
            >
          ...
          {isAuthenticated && (
            <span className="rounded-full bg-[rgba(0,200,83,0.1)] px-3 py-1.5 text-xs font-medium text-[#00c853]">
              Signed In
            </span>
          )} */}
        ...
        {isWrongNetwork && (
          <div className="rounded-2xl border border-[rgba(255,100,0,0.2)] bg-[rgba(255,100,0,0.05)] p-4">
            <p className="text-xs uppercase tracking-[0.24em] text-[var(--color-orange)]">Unsupported Network</p>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {SUPPORTED_CHAIN_LIST.map((cfg) => (
                <button
                  ...
                  className="rounded-full border border-[rgba(0,0,255,0.18)] bg-white px-4 py-2 text-sm font-medium text-[var(--color-cyan)] transition hover:bg-[rgba(0,0,255,0.06)]"
                >
                  {cfg.chain.name}
                </button>
              ))}
